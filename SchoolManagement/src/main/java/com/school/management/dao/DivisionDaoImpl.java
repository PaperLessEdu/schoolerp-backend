...
public class DivisionDaoImpl implements DivisionDao {
	...
	public Division getDivision(Long divison_id) {
		...
		try {
			division = divisionRepository.getOne(divison_id);
		} catch (Exception e) {
	...
	public void deleteDivision(long divion_id) {
		try {
			divisionRepository.deleteById(divion_id);
		} catch (Exception e) {
			String error = String.format("Error occured while deleting standard with divion_id [%s]", divion_id);
			logger.error(error);
			throw e;
		}
